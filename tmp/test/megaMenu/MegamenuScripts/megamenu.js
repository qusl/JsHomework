...
(function () {
    ...
    angular.module('myAppItem', [])
    .controller('mainCtrl', ['$scope', 'MegaMenuService', function ($scope, megaMenuService) {

        (function retrieveMegaMenu() {
            megaMenuService.getMegaMenu().then(getMegaMenu);
        })();

        function getMegaMenu(response) {
            //* Mega menu
    ...
    function generateHtml(displayCategories, parent) {
        ...
        for (var colNum = 0; colNum < displayCategories.length; colNum++) {
            ...
            for (var catNum = 0; catNum < columnCategories.length; catNum++) {
                ...
                title.html(category.IsPublic ? category.CategoryName : ("[Not Public] " + category.CategoryName));
                ...
                for (var subCatNum = 0; subCatNum < category.SubCategories.length; subCatNum++) {
                    ...
                    subcatA.html("" + (subCat.IsPublic ? subCat.CategoryName : ("[Not Public] " + subCat.CategoryName)));//removed 2 spaces

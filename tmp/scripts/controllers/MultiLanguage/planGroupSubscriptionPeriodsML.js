...
(function () {
    var tableName = "PlanGroupSubscriptionPeriodsML";
    var currentPlanGroupDispName = "";
    var currentLangDispName = '';
    var resellerId;

    window.CmpTool.getAllPlanGroupSubscriptionPeriodsML = function (scope, parentresellerId) {
    ...
    function setDataSource(scope) {
        ...
        var refreshGrid = function () {
            var getMethod = "GetPlanGroupSubscriptionPeriodsMLByResellerId";
            scope.getServiceData(tableName, getMethod, resellerId).then(function (respRefresh) {
                myGrid.columns = window.CmpTool.configPlanGroupSubscriptionPeriodsMLColumns(scope);

...
(function () {
    var tableName = "PlanAttributeML";
    var currentPlanDispName = '';
    var currentAttrDetail = "";
    var currentLangDispName = '';
    var resellerId;

    window.CmpTool.getAllPlanAttributeML = function (scope, parentresellerId) {
    ...
    function setDataSource(scope) {
        ...
        myGrid.onCellPrepared = function (e) {
            ...
            if (e.rowType === "detail" && !scope.appIsInsert) {
                ...
                CmpToolUtility.setReadOnlyValue(editorElement, 1, currentAttrDetail);
        ...
        myGrid.onEditingStart = function (e) {
            ...
            var currentData = CmpToolUtility.searchDataTable(scope.planAttributeDetailDropdown, e.data.PADID, 'PADID');
            if (currentData.length > 0) {
                currentAttrDetail = currentData[0].DispName;
            } else {
                currentAttrDetail = '';
            }
        ...
        var refreshGrid = function () {
            var getMethod = "GetPlanAttributeMLByResellerId";
            scope.getServiceData(tableName, getMethod, resellerId).then(function (respRefresh) {
                myGrid.columns = window.CmpTool.configPlanAttributeMLColumns(scope);
